...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.reportparameterService.search({
        ...
        this.reportparameterService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
            (res: ResponseWrapper) => this.onSuccess(res.json, res.headers),
    ...
    transition() {
        this.router.navigate(['/reportparameter'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/reportparameter', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: Reportparameter) {
        return item.id;
    }
    registerChangeInReportparameters() {
        this.eventSubscriber = this.eventManager.subscribe('reportparameterListModification', (response) => this.loadAll());
    }
